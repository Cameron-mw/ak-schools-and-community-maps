...
def update_figure(selected_category, search):
    ...
    # Parameters for map showing all Communities by Incorporation Type
        ...
        text_display = html.H2('Subsistence Use Communities'), \
            'The points on the map represent Alaskan communities designated as subsistence use according to the table \"Subsistence Use Communities\" as of 2017. ' \
        ...
        text_display = html.H2('Environmentally Threatened Communities'), \
            ...
            'Engineers and the University of Alaska Fairbanks (gis.data.alaska.gov). Where available, each threatened community has individual ranking for erosion, flooding, and permafrost degradation risk, along with a summary of hazards. ' \
            'This data was gathered from the \"Environmentally Threatened Communities\" table on the State of Alaska Open Data Geoportal.', \
            html.Br(), html.Br(), \
        ...
        text_display = html.H2('Road Connection by Community'), \
            'The points on the map represent Alaskan communities categorized by thier connection to Alaska\'s road system as designated in the \"Road Connection in Community\" table. The \'road system\' in this case ' \
            'should be thought of as the roads which link communities with the larger highways in Alaska that reach all major ' \
            'population centers in the state. This also includes the Alcan Highway which passes into Canada and eventually' \
            'connects to the lower 48 states. Air, sea, and river are the main alternative modes of transportaion used to access communities without road connection.', \
            html.Br(), html.Br(), \
            'The pie chart \"Percentage of State Population With Access to the Road System\" below displays the percentage of Alaskans living in communities with access to the road system (as of year not specified).'
    ...
    if selected_category == 'Road_Connection':
        ...
        fig = px.scatter_mapbox(filtered_df,
                                ...
                                hover_data=['2021_Population', 'Borough_Census_Area', 'Incorporation_Type',
                                            'Road_Connection', 'Erosion Ranking', 'Flood Ranking', 'Permafrost Ranking', 'Key Hazard Threat'],
                                center={'lat': 63, 'lon': -152},
                                zoom=3)
        fig.update_layout(mapbox_style=map_style, height=800)
    ...
    # Chart for default dropdown selection
    if selected_category == 'Community':
        chart = px.bar(data_frame=pd.read_csv('data/UninhabitedComms.csv'),
                       ...
                       category_orders={'Status':['Uninhabited', 'Under One Hundred (1-99)', 'Under One Thousand (100-999)','Over One Thousand (1000+)']},
